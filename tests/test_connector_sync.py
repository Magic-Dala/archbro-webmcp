...
def _cursor_store() -> SqliteSyncCursorStore:
    return SqliteSyncCursorStore(str(Path(tempfile.mkdtemp()) / "sync.db"))


def test_first_sync_starts_from_no_position_and_records_where_it_stopped():
...
def test_next_sync_resumes_from_the_stored_position():
    store = _cursor_store()
...
def test_replayed_signals_still_allow_the_cursor_to_advance():
    store = _cursor_store()
...
def test_unfinished_signal_holds_the_cursor_back_so_the_next_sync_retries():
    store = _cursor_store()
...
def test_rejected_signal_does_not_deadlock_the_connector():
...
def test_a_connector_that_never_finishes_is_reported_as_stalled():
...
def test_a_successful_pass_clears_the_stall_counter():
    store = _cursor_store()
    store.save(SyncCursor(project_id="proj_1", connector_id="github", position="start"))
...
def test_transport_failure_leaves_the_cursor_untouched():
    store = _cursor_store()
    store.save(SyncCursor(project_id="proj_1", connector_id="github", position="start"))
...
def test_empty_result_does_not_lose_the_existing_position():
    store = _cursor_store()
    store.save(SyncCursor(project_id="proj_1", connector_id="github", position="start"))
