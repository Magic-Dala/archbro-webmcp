...
def make_client(dsn) -> TestClient:
    ...
    return TestClient(build_app(repo, FakeModelProvider()), base_url="http://127.0.0.1:8012")
...
def test_microsoft_teams_pkce_start_uses_tenant_specific_authority(dsn, monkeypatch):
    ...
    assert query["redirect_uri"] == ["http://127.0.0.1:8012/mcp/oauth/microsoft-teams/callback"]
    assert query["prompt"] == ["select_account"]
    assert query["code_challenge_method"] == ["S256"]
    assert "code_challenge" in query
    assert "Chat.Read" in query["scope"][0]
    assert "ChatMessage.Send" not in query["scope"][0]
    assert "ChannelMessage.Send" not in query["scope"][0]
...
def test_microsoft_teams_callback_creates_local_graph_adapter_without_token_leak(dsn, monkeypatch):
    ...
    assert probe.json()["tool_count"] == 5
...
def test_microsoft_teams_adapter_maps_graph_requests_and_keeps_token_in_headers(dsn, monkeypatch):
    ...
    adapter = teams_module.MicrosoftTeamsGraphAdapter("teams-access-secret", enable_write=True)
...
def test_microsoft_teams_write_scopes_and_tools_are_explicit_opt_in(dsn, monkeypatch):
    monkeypatch.setenv("ARCHBRO_MICROSOFT_TEAMS_CLIENT_ID", "teams-public-client")
    monkeypatch.setenv("ARCHBRO_MICROSOFT_TEAMS_TENANT_ID", "tenant-123")
    monkeypatch.setenv("ARCHBRO_TEAMS_ENABLE_WRITE", "true")
    monkeypatch.delenv("ARCHBRO_MICROSOFT_TEAMS_CLIENT_SECRET", raising=False)

    started = make_client(dsn).get("/mcp/oauth/microsoft-teams/start", follow_redirects=False)
    scope = set(parse_qs(urlparse(started.headers["location"]).query)["scope"][0].split())
    assert {"ChatMessage.Send", "ChannelMessage.Send"}.issubset(scope)

    adapter = teams_module.MicrosoftTeamsGraphAdapter("teams-access-secret")
    assert len(adapter.list_tools()) == 5
    try:
        adapter.call_tool("teams_send_chat_message", {"chat_id": "chat-1", "content": "hello"})
    except ValueError as exc:
        assert "write tools are disabled" in str(exc)
    else:
        raise AssertionError("Teams write tools must be opt-in")

    write_adapter = teams_module.MicrosoftTeamsGraphAdapter("teams-access-secret", enable_write=True)
    assert len(write_adapter.list_tools()) == 7
